...
이 스킬들은 프로젝트 내에서 로컬로 유지되며 skill-hub로 관리되지 않습니다:
...
### 새 스킬 설치 시 (skill-hub 사용)

`skill-hub install` 명령어를 사용할 때 **자동으로 이 파일이 업데이트됩니다**:
```bash
python skills/skill-hub/scripts/hub.py install --git-url "https://github.com/owner/repo"
