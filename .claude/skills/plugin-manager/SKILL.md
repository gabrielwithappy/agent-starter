...
**Output:**
```
?ì¶ 'obsidian-skills' ?§Ïö¥Î°úÎìú Ï§?..
???åÎü¨Í∑∏Ïù∏ 'obsidian-skills'??Í∞Ä) ?±Í≥µ?ÅÏúºÎ°??§Ïπò?òÏóà?µÎãà??
??3Í∞úÏùò skill???§Ïπò?òÏóà?µÎãà??
...
**Output:**
```
?óëÔ∏? 'json-canvas' ?úÍ±∞ Ï§?..
???§ÌÇ¨ 'json-canvas'??Í∞Ä) ?±Í≥µ?ÅÏúºÎ°??úÍ±∞?òÏóà?µÎãà??
??5Í∞úÏùò ?åÏùº????†ú?òÏóà?µÎãà??
```
...
```
plugin-manager/
?ú‚??Ä SKILL.md              # This file
?ú‚??Ä README.md             # Quick start guide
?ú‚??Ä scripts/              # Executable scripts
??  ?ú‚??Ä manage.py        # Main CLI script
??  ?ú‚??Ä example.py       # Usage examples
??  ?î‚??Ä validate.py      # Validation script
?ú‚??Ä assets/               # Data and templates
??  ?î‚??Ä registry.json    # Installation registry
?î‚??Ä references/           # Reference documentation
    ?ú‚??Ä QUICKREF.md      # Quick reference card
    ?ú‚??Ä MIGRATION.md     # Migration guide
    ?î‚??Ä COMPLETION.md    # Completion report
```
