...
Find notes with status equals "ÏßÑÌñâÏ§?:
```bash
python scripts/frontmatter_search.py "d:\00_MyData\obsidianKMS" --property "?ÅÌÉú" --equals "ÏßÑÌñâÏ§?
```

Multiple filters (AND logic):
```bash
python scripts/frontmatter_search.py "d:\00_MyData\obsidianKMS" \
  --property "tags" --contains "22_?ÖÎ¨¥Í≤ΩÎ†•/Í∞úÎ∞ú/AI" \
  --property "?ÅÌÉú" --equals "?ïÎ≥¥"
```
...
```bash
python scripts/frontmatter_list.py "d:\00_MyData\obsidianKMS" --property "?ÅÌÉú" --min-count 5
```
...
```bash
python scripts/frontmatter_modify.py "note.md" --update "?ÅÌÉú:?ÑÎ£å" --apply
```
...
```bash
python scripts/frontmatter_list.py "d:\00_MyData\obsidianKMS" --property "?ÅÌÉú"
```
